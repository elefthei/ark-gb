...
//! reference output.
//!
//! The fixtures in `tests/fixtures/*.gb.txt` were generated by
//! Singular 4.x with `option(redSB); sort(std(I))` and committed to
//! the ark_gb repo so these tests don't depend on Singular being
//! installed. See the task's plan for how to regenerate them.
//!
//! The parser here understands Singular's output format for
//! degrevlex over Z/p: one polynomial per line, terms separated by
//! `+` or `-`, each term is an optional coefficient followed by
//! zero or more `name[exp][*name[exp]]*` variable factors. `name2`
//! and `name^2` are both accepted (Singular emits the former for
//! single-letter variables like `x` and the latter for names with
//! digits like `u0`). Implicit multiplication between factors is
//! required (Singular writes it when adjacent factors don't need
//! disambiguation).
...
impl<'a> LineParser<'a> {
    fn new(src: &'a str, ring: &'a Ring, var_names: &'a [&'a str]) -> Self {
        Self {
            src: src.as_bytes(),
            pos: 0,
            ring,
    ...
    fn parse(mut self) -> Poly {
        let p = self.ring.field().p();
        let mut terms: Vec<(Coeff, Monomial)> = Vec::new();
        let mut first = true;
        loop {
            match self.read_term(first) {
                ...
                Some((sign, coeff_opt, exps)) => {
                    ...
                    // Reduce mod p and apply sign.
                    let mag_mod = (mag % p as u64) as u32;
                    let signed = if sign < 0 {
                        // negate mod p
                        if mag_mod == 0 { 0 } else { p - mag_mod }
                    } else {
                        mag_mod
                    };
                    if signed != 0 {
...
fn parse_fixture(text: &str, ring: &Ring, var_names: &[&str]) -> Vec<Poly> {
    text.lines()
        .filter_map(|line| {
...
fn sort_gb_ascending(mut gb: Vec<Poly>, ring: &Ring) -> Vec<Poly> {
    gb.sort_by(|a, b| {
...
fn cyclic4_matches_singular_fixture() {
    ...
    // abcd - 1
    let f4 = Poly::from_terms(&r, vec![(1, m(&[1, 1, 1, 1])), (32002, m(&[0, 0, 0, 0]))]);
